...
FRA_BASE_URL = os.environ.get(
    "FRA_BASE_URL", "http://licensing.clinicnetworking.com:8000"
)
...
def list_assignments(jwt_token: str) -> Dict[str, Any]:
    ...
    try:
        resp = requests.post(
            ASSIGNMENTS_LIST_URL, headers=_headers_with_jwt(jwt_token), timeout=10
        )
...
def request_assignments(
    jwt_token: str, numbers: List[str], device_id: str | None = None
) -> Dict[str, Any]:
    """Call /assignments.request for the given numbers."""
    try:
        payload = {"device_id": device_id or get_device_id(), "numbers": numbers}
        resp = requests.post(
            ASSIGNMENTS_REQUEST_URL,
            headers=_headers_with_jwt(jwt_token),
            json=payload,
            timeout=10,
        )
        if resp.status_code != 200:
...
def unregister_assignments(
    jwt_token: str, numbers: List[str] | None = None, device_id: str | None = None
) -> Dict[str, Any]:
