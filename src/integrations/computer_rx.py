...
class CRxIntegration2(QThread):
    ...
    def __init__(self, base_dir: str, parent=None):
        ...
        self.dll_path2 = (
            r"C:\\Program Files (x86)\\Pervasive Software\\PSQL\\bin\\wbtrv32.dll"
        )
        # Btrieve constants
        self.B_OPEN = 0
        self.B_GET_FIRST = 12
    ...
    def _check_enabled(self) -> bool:
        try:
            ...
            dev_settings = (
                device_config.get("Integrations", "integration_settings", {}) or {}
            )
            ...
            if not winrx_path:
                self.log.debug(
                    "Computer-Rx enabled but WinRx path not configured; skipping."
                )
                return False
            return True
        except Exception:
    ...
    def run_initial_cleanup_if_needed(parent=None):
        ...
        try:
            ...
            already_done = (
                global_config.get("Integrations", "crx_initial_cleanup_done", False)
                or False
            )
            if already_done:
                return
            ...
            if not (winrx_path and os.path.isdir(winrx_path)):
                ...
                log.debug(
                    "Computer-Rx initial cleanup: WinRx path not configured or invalid; deferring."
                )
                return
            ...
            if not btrieve_file:
                log.debug(
                    "Computer-Rx initial cleanup: FaxControl.btr not found; nothing to do."
                )
            ...
            dll_to_load = (
                ...
                else (dll_path2 if os.path.exists(dll_path2) else None)
            )
            if not dll_to_load:
                log.debug(
                    "Computer-Rx initial cleanup: Btrieve DLL not found; deferring until installed."
                )
                return
            ...
            try:
                ...
                log.debug(
                    f"Computer-Rx initial cleanup: Failed to load Btrieve DLL: {e}; deferring."
                )
                return

            status = btrieve.BTRCALL(
                B_OPEN,
                POSITION_BLOCK,
                DATA_BUFFER,
            ...
            if status != 0:
                log.warning(
                    f"Computer-Rx initial cleanup: Failed to open {btrieve_file}; status {status}"
                )
            ...
            try:
                ...
                if status != 0 and status != B_EOF:
                    log.warning(
                        f"Computer-Rx initial cleanup: GET_FIRST returned status {status}"
                    )
                    ...
                    if response == QMessageBox.Yes:
                        ...
                        while status == 0:
                            ...
                            try:
                                ...
                                file_name = ""
                            full_file_path = (
                                os.path.join(winrx_path, file_name) if file_name else ""
                            )
                            # Delete record first
                            del_status = btrieve.BTRCALL(
                                B_DELETE,
                                POSITION_BLOCK,
                                DATA_BUFFER,
                                ctypes.byref(data_length),
                            ...
                            if del_status != 0:
                                log.warning(
                                    f"Computer-Rx initial cleanup: Failed to delete a record; status {del_status}"
                                )
                            ...
                            if full_file_path and os.path.exists(full_file_path):
                                try:
                                    ...
                                    log.warning(
                                        f"Computer-Rx initial cleanup: Failed to delete file {full_file_path}: {de}"
                                    )
                            # Next
                            status = btrieve.BTRCALL(
                                B_GET_NEXT,
                            ...
                            if status == B_EOF:
                                break
                        log.info(
                            "Computer-Rx initial cleanup: Cleared existing Btrieve records and files."
                        )
                    else:
                        log.info(
                            "Computer-Rx initial cleanup: User declined to clear existing records."
                        )
                else:
    ...
    def _process_records(self):
        ...
        if not fax_user:
            try:
                self.log.error(
                    "Computer-Rx: fax_user missing from config; skipping run until account is configured."
                )
            except Exception:
                pass
            return
        ...
        caller_id = app_state.device_cfg.selected_fax_number or (
            app_state.device_cfg.selected_fax_numbers[0]
            if app_state.device_cfg.selected_fax_numbers
            else (
        ...
        if len(caller_id) < 10:
            self.log.warning(
                "Computer-Rx: Caller ID appears invalid/missing; proceeding but number formatting may fail."
            )
        winrx_path = device_config.get("Integrations", "winrx_path", "") or ""
        ...
        if not btrieve_file:
            self.log.error(
                "Computer-Rx: FaxControl.btr not found in WinRx path; skipping run."
            )
            return
        # Load Btrieve dll
        dll_to_load = (
            self.dll_path1
            if os.path.exists(self.dll_path1)
            else (self.dll_path2 if os.path.exists(self.dll_path2) else None)
        )
        if not dll_to_load:
            self.log.error(
                "Computer-Rx: Btrieve DLL not found. Ensure Actian/Pervasive PSQL is installed."
            )
            return
        try:
            btrieve = ctypes.WinDLL(dll_to_load)
        ...
        if status != 0:
            self.log.error(
                f"Computer-Rx: Failed to open Btrieve file: {btrieve_file} | Status: {status}"
            )
            return
        try:
            # Iterate records
            ...
            while status == 0:
                ...
                if len(dest) != 11:
                    self.log.warning(
                        f"Computer-Rx: Invalid destination number for record {record_id}; skipping."
                    )
                    status = btrieve.BTRCALL(
                        self.B_GET_NEXT,
                        self.POSITION_BLOCK,
                ...
                if not (full_file_path and os.path.exists(full_file_path)):
                    self.log.warning(
                        f"Computer-Rx: Missing fax file for record {record_id}: {full_file_path}"
                    )
                    status = btrieve.BTRCALL(
                        self.B_GET_NEXT,
                        self.POSITION_BLOCK,
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        self.log.info(
                            f"Computer-Rx: Fax sent (record {record_id}) to {dest}; removing record and file."
                        )
                        del_status = btrieve.BTRCALL(
                            self.B_DELETE,
                            self.POSITION_BLOCK,
                        ...
                        if del_status != 0:
                            self.log.warning(
                                f"Computer-Rx: Failed to remove record {record_id} from Btrieve. Status {del_status}"
                            )
                        try:
                            os.remove(full_file_path)
                        except Exception as de:
                            self.log.warning(
                                f"Computer-Rx: Failed to delete file {full_file_path}: {de}"
                            )
                    else:
                        self.log.error(
                            f"Computer-Rx: Send failed for record {record_id} -> {dest}; HTTP {resp.status_code} {resp.text}"
                        )
                except Exception as se:
                    self.log.error(
                        f"Computer-Rx: Error sending record {record_id}: {se}"
                    )
                finally:
                    try:
            ...
            try:
                btrieve.BTRCALL(
                    self.B_CLOSE, self.POSITION_BLOCK, None, None, None, 0, None
                )
            except Exception:
                pass
